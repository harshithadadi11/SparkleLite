...
const REQUIRED_DISCLAIMER =
  'This is not a medical diagnosis. This app cannot diagnose conditions or replace professional medical advice. Please consult a qualified healthcare provider about any health concerns.';
...
export function aiInsightEngine(logs: any[], userId: string): Omit<AIInsight, 'id'> {
  ...
  const irregularBleeding = logs.some(log =>
    Array.isArray(log.symptoms) && log.symptoms.includes('irregularBleeding')
  );
  const hasDizzinessNote = logs.some(log =>
    typeof log.notes === 'string' &&
    (log.notes.toLowerCase().includes('dizzy') || log.notes.toLowerCase().includes('dizziness'))
  );
  const hasNoSymptoms = logs.every(log =>
    (!log.symptoms || log.symptoms.length === 0) && log.painLevel <= 2
  );
  ...
  if (highPainLogs.length > 0) {
    ...
    doctorQuestions.push(
      'What might be causing recurring pain at this level?',
      'Should I track any additional details about my pain?',
      'Are there any tests you would recommend?',
      'When should I seek emergency care for pain?'
    );
  }
  ...
  if (heavyFlowLogs.length > 0 && hasDizzinessNote) {
    ...
    doctorQuestions.push(
      'Could heavy flow be affecting my iron levels or causing dizziness?',
      'What level of flow is considered medically heavy?',
      'Should I have a blood count test?'
    );
  }
  ...
  if (irregularBleeding) {
    ...
    doctorQuestions.push(
      ...
      'Is the timing or frequency of irregular bleeding significant?',
      'Are there any lifestyle factors that might be contributing?',
  ...
  if (manyRecentLogs && !summary) {
    ...
    doctorQuestions.push(
      'Does my logged history show any patterns I should be aware of?',
      'Are my symptoms within a typical range?',
      'How often should I be having routine check-ups?'
    );
  }
  ...
  if (hasNoSymptoms || !summary) {
    ...
    doctorQuestions.push(
      ...
      'What symptoms should I watch for and log?',
      'Is my current cycle pattern typical?'
    );
  }
